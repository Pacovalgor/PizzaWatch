...
class IngredientesController: WKInterfaceController {
    ...
    @IBAction func continuar() {
        ...
        if(cont<1){
            ...
            self.presentAlertControllerWithTitle("Advertencia!!", message: "Debes elegir al menos 1 ingrediente", preferredStyle: WKAlertControllerStyle.Alert, actions: [ok])
        }
        else if(cont>5){
            let ok = WKAlertAction(title: "OK", style: WKAlertActionStyle.Cancel, handler: { () -> Void in })
            self.presentAlertControllerWithTitle("Advertencia!!", message: "Debes elegir 5 ingredientes como máximo", preferredStyle: WKAlertControllerStyle.Alert, actions: [ok])
        }
        else{
